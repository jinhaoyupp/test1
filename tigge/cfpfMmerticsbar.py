...
data = pd.read_excel('/Users/jinhaoyu/Downloads/Chinasurfacepcpv2/dataprocess/tigge/apf2HB5_9/cfVSpfmetrics.xlsx', sheet_name=7, header=0)
# 0-7
datav = data.values
cfv = datav[0:1, 1:8]
pfv = datav[1:2, 1:8]
...
bar1 = plt.bar([i - 0.2 for i in x], height = cfvl[0], width = 0.4,
               alpha = 0.8, color = 'r',label = 'CF')                  #第一个图

bar2 = plt.bar([i + 0.2 for i in x],height=pfvl[0],width = 0.4,
               alpha = 0.8,color = 'g',label = 'PF')                   #第二个图

plt.xticks(x, namemodel)      #设置x轴刻度显示值
# plt.ylim(0, 10500)       #y轴的范围
plt.title('(h)', fontsize=13)     #标题
plt.xlabel('Models', fontsize=13)       #x轴的标签
plt.ylabel('OFE(mm)', fontsize=13)       #y轴的标签
plt.legend(fontsize=13)            #设置图例
...
for rect in bar1:
    ...
    plt.text(rect.get_x() + rect.get_width() / 2, height*1.015, '%.2f' % height, ha="center", va="bottom", fontsize=13)
for rect in bar2:
